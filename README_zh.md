...
*   **RSA/Ed25519 金鑰產生器**：可產生 RSA 或 Ed25519 金鑰對，並把公鑰輸出檔案預設寫成 OpenSSH 相容的 `.pub` 格式。切換金鑰類型時，會自動把預設輸出檔名同步為 RSA 的 `id_rsa.pem` / `id_rsa.pub`，或 Ed25519 的 `id_ed25519.pem` / `id_ed25519.pub`，瀏覽對話框也會沿用目前的預設值。也可勾選額外把同一份 OpenSSH 公鑰複製到剪貼簿。
*   **雜湊值工具 (Hash Utility)**：計算純文字與檔案的 MD5、CRC32、SHA-1、SHA-256 和 SHA3-256 雜湊值。可透過將計算出的雜湊值與現有的校驗檔進行比對，輕鬆驗證檔案完整性。
...
*   `crc32fast`（用於 CRC32 雜湊計算）(MIT/Apache 2.0)
